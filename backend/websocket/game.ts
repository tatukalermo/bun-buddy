type UnitVariation = "basic";
type Location = { x: number; y: number };
type PlayerSide = "player1" | "player2";
type Player = { username: string; side: PlayerSide };
type Unit = {
  owner: PlayerSide;
  ...
  return Bun.serve<Player>({
    ...
    websocket: {
      ...
      open(ws) {
        ...
        ws.publish(
          "lobby",
          JSON.stringify({ type: "USERS_ADD", data: newUser })
        );
      ...
      message(ws, msg: string) {
        ...
        ws.send(
          JSON.stringify({ type: "GAMESTATE_SET", data: serverGameState })
        );
      },
